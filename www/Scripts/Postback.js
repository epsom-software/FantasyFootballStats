...
(function (Postback) {
    ...
    function submit() {
        ...
        $.getJSON("Node/server.js", { code: query }, callback);
        return false;
    }

    function init() {
        ["TotalPoints", "Cost", "TypeName", "TeamName", "Form", "EventPoints", "NextFixture", "News"].forEach(function (f) {
            return $(".Fields label:contains('" + f + "')").addClass("on");
        });
